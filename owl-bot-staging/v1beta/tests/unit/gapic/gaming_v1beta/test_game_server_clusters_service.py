...
def test_list_game_server_clusters_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert path_template.validate("%s/v1beta/{parent=projects/*/locations/*/realms/*}/gameServerClusters" % client.transport._host, args[1])
...
def test_get_game_server_cluster_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{name=projects/*/locations/*/realms/*/gameServerClusters/*}" % client.transport._host, args[1])
...
def test_create_game_server_cluster_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/locations/sample2/realms/sample3'}
    request_init["game_server_cluster"] = {'name': 'name_value', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'connection_info': {'gke_cluster_reference': {'cluster': 'cluster_value'}, 'namespace': 'namespace_value'}, 'etag': 'etag_value', 'description': 'description_value'}
    request = request_type(**request_init)
...
def test_preview_create_game_server_cluster_rest_required_fields(request_type=game_server_clusters.PreviewCreateGameServerClusterRequest):
    ...
    unset_fields = transport_class(credentials=ga_credentials.AnonymousCredentials()).preview_create_game_server_cluster._get_unset_required_fields(jsonified_request)
    # Check that path parameters and body parameters are not mixing in.
    assert not set(unset_fields) - set(("game_server_cluster_id", "preview_time", ))
    jsonified_request.update(unset_fields)
...
def test_preview_create_game_server_cluster_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(("gameServerClusterId", "previewTime", )) & set(("parent", "gameServerClusterId", "gameServerCluster", )))
...
def test_update_game_server_cluster_rest(request_type):
    ...
    request_init["game_server_cluster"] = {'name': 'projects/sample1/locations/sample2/realms/sample3/gameServerClusters/sample4', 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'labels': {}, 'connection_info': {'gke_cluster_reference': {'cluster': 'cluster_value'}, 'namespace': 'namespace_value'}, 'etag': 'etag_value', 'description': 'description_value'}
    request = request_type(**request_init)
...
def test_update_game_server_cluster_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        assert len(req.mock_calls) == 1
        _, args, _ = req.mock_calls[0]
        assert path_template.validate("%s/v1beta/{game_server_cluster.name=projects/*/locations/*/realms/*/gameServerClusters/*}" % client.transport._host, args[1])
...
def test_game_server_clusters_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.gaming_v1beta.services.game_server_clusters_service.transports.GameServerClustersServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_game_server_clusters_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.gaming_v1beta.services.game_server_clusters_service.transports.GameServerClustersServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.GameServerClustersServiceTransport()
        adc.assert_called_once()
