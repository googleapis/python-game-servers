...
from google.cloud.gaming_v1.types import common, game_server_configs
...
def test_game_server_configs_service_client_get_mtls_endpoint_and_cert_source(
    client_class,
):
...
def test_game_server_configs_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
async def test_list_game_server_configs_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_game_server_configs(
            request={},
        )
...
def test_list_game_server_configs_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        assert path_template.validate(
            "%s/v1/{parent=projects/*/locations/*/gameServerDeployments/*}/configs"
...
def test_get_game_server_config_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), "request") as req:
        ...
        _, args, _ = req.mock_calls[0]
        assert path_template.validate(
            "%s/v1/{name=projects/*/locations/*/gameServerDeployments/*/configs/*}"
            % client.transport._host,
            args[1],
        )
...
def test_game_server_configs_service_base_transport():
    ...
    with mock.patch(
        "google.cloud.gaming_v1.services.game_server_configs_service.transports.GameServerConfigsServiceTransport.__init__"
    ) as Transport:
...
def test_game_server_configs_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(
        ...
        "google.cloud.gaming_v1.services.game_server_configs_service.transports.GameServerConfigsServiceTransport._prep_wrapped_messages"
    ) as Transport:
        Transport.return_value = None
...
def test_game_server_configs_service_transport_create_channel(
    transport_class, grpc_helpers
):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
